...
function resizeRenderer(){
  ...
  renderer.domElement.style.left = (width - size) / 2 + "px";
  renderer.domElement.style.top = (height - size) / 2 + "px";

  camera.aspect = 1; // square scene
...
new THREE.TextureLoader().load("garden.png", (texture) => {
...
function fetchAirportState(code){
  ...
  const airportData = {
    ...
    BLR: { parked: 7, arrivals: 5, departures: 5 },
    
    DEL: { parked: 6, arrivals: 3, departures: 4 },

    BOM: { parked: 5, arrivals: 2, departures: 3 },

    MAA: { parked: 7, arrivals: 1, departures: 2 },

    HYD: { parked: 4, arrivals: 2, departures: 1 }
  };
...
function updateUI(code, state){
  ...
  counter.innerHTML = `
    Airport: ${code}<br><br>
    Parked: ${state.parked}<br>
    Arrivals: ${state.arrivals}<br>
